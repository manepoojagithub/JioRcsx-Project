...
public sealed class JioCxClient : IJioCxClient
{
    ...
    public JioCxClient(HttpClient httpClient, IOptions<JioCxOptions> options)
    {
    ...
    public async Task<JioCxUploadResult> UploadFileAsync(string apiKey, string agentId, Stream file, string fileName, string contentType, CancellationToken cancellationToken)
    {
        try
        {
            ...
            using var request = new HttpRequestMessage(HttpMethod.Post, _options.UploadFilePath);
            ...
            return new JioCxUploadResult(response.IsSuccessStatusCode, (int)response.StatusCode, responseJson, TryReadUrl(responseJson));
        }
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            return new JioCxUploadResult(false, 0, "JioCX media upload timed out.", null);
        }
        catch (HttpRequestException ex)
        {
            return new JioCxUploadResult(false, 0, BuildTransportFailure(ex), null);
        }
    ...
    public async Task<JioCxSendResult> SendMessageAsync(string apiKey, JioCxSendRequest requestModel, CancellationToken cancellationToken)
    {
        try
        {
            using var request = new HttpRequestMessage(HttpMethod.Post, _options.SendMessagePath);
            request.Headers.Add("x-apikey", apiKey);
            var payload = new
            {
            ...
            using var response = await _httpClient.SendAsync(request, cancellationToken);
            return new JioCxSendResult(response.IsSuccessStatusCode, (int)response.StatusCode, await response.Content.ReadAsStringAsync(cancellationToken));
        }
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            return new JioCxSendResult(false, 0, "JioCX send message request timed out.");
        }
        catch (HttpRequestException ex)
        {
            return new JioCxSendResult(false, 0, BuildTransportFailure(ex));
        }
    }
